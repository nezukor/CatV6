		...
		if (not teleportedServers) and (not shared.VapeIndependent) and vape.AutoTeleport.Enabled then
			...
			local teleportScript = [[
				...
				if shared.VapeDeveloper then
					...
					loadstring(game:HttpGet('https://raw.githubusercontent.com/nezukor/CatV6/'..readfile('catrewrite/profiles/commit.txt')..'/init.lua', true), 'init')()
				end
			]]
			if shared.VapeDeveloper then
...
if not shared.VapeIndependent then
	...
	if not found then
		local suc, res = pcall(function()
			return not shared.VapeDeveloper and game:HttpGet('https://raw.githubusercontent.com/nezukor/CatV6/'..readfile('catrewrite/profiles/commit.txt')..'/games/'..game.PlaceId..'.lua', true) or '404: Not Found'
		end)
